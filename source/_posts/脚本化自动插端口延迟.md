...
if(keys(%sig_clkrst_hashof) != 2) {
    die "Must has one clock and one reset: ", %sig_clkrst_hashof;
}
...
my $param_decl = "!!!You should set parameter!!!";
...
module ${mod_name}_pipeline #($param_decl) (
$interface_decl
);

$wires_decl
$pipeline_decl
    ${mod_name} #($param_decl) U_\U${mod_name}\E (
$inst_decl
    );
...
module match_proc_pipeline #(!!!You should set parameter!!!) (
    input   clock,
    input   reset_n,
    input  [CFG_BLK-1:0] cfg_lut,
    input  [CHNL_WIDTH-1:0] drm_sink_channel,
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(CFG_BLK-1-0+1),.PIPE_NUM(0)) IN_NDELAY_CFG_LUT (
        .clk     (clock),
        .rst_n   (reset_n),
        .sig_din (cfg_lut),
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(CHNL_WIDTH-1-0+1),.PIPE_NUM(0)) IN_NDELAY_DRM_SINK_CHANNEL (
        .clk     (clock),
        .rst_n   (reset_n),
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(BUS_WIDTH-1-0+1),.PIPE_NUM(0)) IN_NDELAY_DRM_SINK_DATA (
        .clk     (clock),
        .rst_n   (reset_n),
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(1),.PIPE_NUM(0)) IN_NDELAY_DRM_SINK_EOP (
        .clk     (clock),
        .rst_n   (reset_n),
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(1),.PIPE_NUM(0)) IN_NDELAY_DRM_SINK_SOP (
        .clk     (clock),
        .rst_n   (reset_n),
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(1),.PIPE_NUM(0)) IN_NDELAY_DRM_SINK_VALID (
        .clk     (clock),
        .rst_n   (reset_n),
    ...
    signal_ndelay #(.RST_MODE(0),.DWIDTH(1),.PIPE_NUM(0)) IN_NDELAY_DRM_SOURCE_READY (
        .clk     (clock),
        .rst_n   (reset_n),
    ...
    match_proc #(!!!You should set parameter!!!) U_MATCH_PROC (
        .clock(clock), 
        .reset_n(reset_n), 
